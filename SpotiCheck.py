...
class Output(object):
    ...
    def Save_html(self, accounts):
        ...
        try:
            ...
            html = "<html>\n<head>\n<title>SpotiCheck HTML Result</title>\n</head>"
            ...
            html += "<h3>Result generated by SpotiCheck (www.github.com/JhonnyLusonode/SpotiCheck)</h3>\n"
...
""".format(VERSION))
    ...
    def start_check(self):
        ...
        if not self.nothread:
            ...
            try:
                for _ in tqdm(pool.imap_unordered(self.SpotiCheck, self.accounts_array), total=len(self.accounts_array), desc="Processing accounts"):
                    pass
...
parser.add_argument("--threads", help="Number of workers that SpotiCheck uses (A very high number can cause an error in the program due to the limitations of your computer) (Default: {}).".format(cpu_count()), type=int, action="store")
parser.add_argument("--nothreads", help="If this argument is specified, SpotiCheck will not create any thread, otherwise the main SpotiCheck process will perform the checks.", action="store_true", default=False)
